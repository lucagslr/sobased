...
/** Cards grouped by workspace (in the order of the sidebar), so that the
 * container of every root project stays visible. */
const groups = computed(() => {
  const byWorkspace = new Map<number, ProjectCard[]>()
  for (const card of cards.value ?? []) {
    byWorkspace.set(card.workspace, [...(byWorkspace.get(card.workspace) ?? []), card])
  }
  return [...workspaces.items]
    .sort((a, b) => a.name.localeCompare(b.name, 'fr'))
    .filter((workspace) => byWorkspace.has(workspace.id))
    .map((workspace) => ({ workspace, cards: byWorkspace.get(workspace.id) ?? [] }))
})
...
<template>
  ...
  <div v-else class="space-y-8">
    <section v-for="group in groups" :key="group.workspace.id" class="space-y-4">
      <h2 class="flex items-center gap-2 text-xs font-semibold tracking-wide text-muted uppercase">
        <ColorDot :color="group.workspace.color" /> Espace {{ group.workspace.name }}
      </h2>
      <article
            ...
            <ul class="space-y-2">
              <li v-for="entry in card[key]" :key="entry.id">
                ...
                >
                  ...
                  <span
                    class="mt-2 flex flex-wrap items-center gap-x-2 gap-y-1.5 text-xs text-muted"
                  >
                  ...
                  >
                    ...
                    <span v-if="entry.children_count" class="text-muted">
                      {{ entry.children_count }} sous-projet{{
                        entry.children_count > 1 ? 's' : ''
                      }}
                    </span>
                  </span>
                </RouterLink>
