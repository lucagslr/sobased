...
- **Sous-projet** : sous-dossier dans le dossier du parent, créé **avec le compte propriétaire de l'arbre** (`folder_owner()` remonte jusqu'à la racine), quel que soit l'utilisateur Faiblegraine qui agit (ses droits Faiblegraine sont vérifiés d'abord). Parent sans dossier → `drive_status: parent_missing`, création refusée avec un message clair.
...
- **Page fichier** : « Nouvelle version depuis Drive » dans le menu, carte Drive de la version avec « Importer dans Faiblegraine ».
