...
| `FERNET_KEY` | Chiffrement des jetons OAuth et de la copie des jetons de liens partagés (obligatoire dès la phase 9 : `python -c "import base64,os; print(base64.urlsafe_b64encode(os.urandom(32)).decode())"`) |
...
| `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, `GOOGLE_API_KEY`, `GOOGLE_APP_ID` | Drive, Picker, Calendar (intégration désactivée si vide). Client OAuth « application Web » avec l'URI de redirection `<SITE_URL>/api/integrations/google/callback/`, API Drive et Picker activées, clé API restreinte au Picker, numéro du projet comme app id |
| `MS_CLIENT_ID`, `MS_CLIENT_SECRET`, `MS_TENANT` | Microsoft Graph, calendrier Outlook / Teams (désactivée si vide). Application Azure « Web » avec l'URI de redirection `<SITE_URL>/api/integrations/microsoft/callback/`, permissions déléguées `Calendars.ReadWrite`, `User.Read`, `offline_access` ; `MS_TENANT` = `common` ou l'id du tenant |
